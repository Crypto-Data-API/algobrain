...
An AI agent connected to the [[cryptodataapi-mcp|CryptoDataAPI MCP]] can trade ICP-PERP end-to-end:

- **Live state** — `GET /api/v1/hyperliquid/summary?coin=ICP` (mark, funding, OI in one call); order-book depth via `GET /api/v1/hyperliquid/l2-book?coin=ICP`
- **Per-coin regime & sizing** — `GET /api/v1/quant/coins/ICP` for this coin's HMM regime state; batch sizing across a book via `GET /api/v1/quant/coins/risk?horizon=24h` (vol-target multiplier)
- **Backtest** — `GET /api/v1/backtesting/funding?symbol=ICP&exchange=hyperliquid` (hourly funding since 2023-05 or listing) and `GET /api/v1/backtesting/klines` (Hyperliquid daily candles back to the 2023 launch or listing; 1m bars since 2026-03-30)
