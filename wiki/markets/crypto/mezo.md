---
title: "MEZO"
type: entity
created: 2026-04-09
updated: 2026-06-12
status: draft
tags: [crypto, defi]
aliases: ["MEZO"]
entity_type: protocol
headquarters: "Decentralized"
website: "https://mezo.org/"
related: ["[[crypto-markets]]"]
---

> *As of 2026-06-12 this asset is outside the CoinGecko top 1000; figures below are the last cached snapshot and should be treated as stale.*

# MEZO

**MEZO** (MEZO) is the onchain Bitcoin banking platform owned by its users. Users can access self-service Bitcoin finance without interacting with loan officers, undergoing credit checks, or observing operational hours. It ranks **#911** by market capitalization.

---

## Key Facts

| Field | Detail |
|---|---|
| **Ticker** | MEZO |
| **Market Cap Rank** | #911 |
| **Market Cap** | $16.39M |
| **Current Price** | $0.0328 |
| **Categories** | Decentralized Finance (DeFi), Lending/Borrowing Protocols, Ethereum Ecosystem, Base Ecosystem, Appchains, BTCfi Protocol, Mezo Ecosystem |
| **Website** | [https://mezo.org/](https://mezo.org/) |

---

## Overview

Mezo is the onchain Bitcoin banking platform owned by its users. Users can access self-service Bitcoin finance without interacting with loan officers, undergoing credit checks, or observing operational hours. It’s just you, your Bitcoin, and permissionless access to financial services 24/7.

Traditional banks profit from your deposits while charging you for access to your own money. Two-sided lending markets trap you with 8-9% variable rates that can spike unpredictably, especially in decentralized financial systems. Both systems put middlemen between you and your wealth.

Mezo eliminates them all, giving you the power to manage your finances intelligently on your own terms.

Deposit Bitcoin as collateral, mint MUSD (a dollar-pegged stablecoin), and use it however you need. Your Bitcoin stays yours while MUSD provides you with instant liquidity. Fixed rates start as low as 1% and stay fixed for the entire life of the loan.

This is self-service banking with complete autonomy. Unlike any traditional banking experience, there are no minimum payments, set repayment schedules, or penalties for early repayment. Close your position tomorrow or keep it open for years.

It’s your Bitcoin and your decision on what to do with it.

---

## Tokenomics

| Metric | Value |
|---|---|
| **Circulating Supply** | 500.00M MEZO |
| **Total Supply** | 1.00B MEZO |
| **Max Supply** | 1.00B MEZO |
| **Fully Diluted Valuation** | $32.78M |
| **Market Cap / FDV Ratio** | 0.50 |

---

## Price History

| Metric | Value |
|---|---|
| **All-Time High** | $0.1128 (2026-04-01) |
| **Current vs ATH** | -71.66% |
| **All-Time Low** | $0.0295 (2026-04-07) |
| **Current vs ATL** | +8.46% |
| **24h Change** | +2.64% |
| **7d Change** | -54.29% |
| **30d Change** | +0.00% |
| **1y Change** | +0.00% |

---

## Platform & Chain Information

**Native Chain:** Mezo

### Contract Addresses

| Chain | Address |
|---|---|
| Mezo | `0x7b7c000000000000000000000000000000000001` |
| Base | `0x8e4cbbcc33db6c0a18561fde1f6ba35906d4848b` |
| Ethereum | `0x8e4cbbcc33db6c0a18561fde1f6ba35906d4848b` |

---

## Exchange Listings

### Centralized Exchanges

| Exchange | Pair | Trust Score |
|---|---|---|
| Kraken | MEZO/USD | N/A |
| Bitget | MEZO/USDT | N/A |
| KuCoin | MEZO/USDT | N/A |

---

## Social & Community

| Platform | Link / Metric |
|---|---|
| **Website** | [https://mezo.org/](https://mezo.org/) |
| **Twitter** | [@MezoNetwork](https://twitter.com/MezoNetwork) |
| **Discord** | [https://discord.com/invite/mezo](https://discord.com/invite/mezo) |

---

## Trading Characteristics

| Characteristic | Detail |
|---|---|
| **24h Volume** | $7.58M |
| **Market Cap Rank** | #911 |
| **24h Range** | $0.0309 — $0.0357 |
| **CoinGecko Sentiment** | 100% positive |
| **Last Updated** | 2026-04-09 |

---

## Whale & Holder Information

> *On-chain holder distribution data requires blockchain analytics integration. This section will be populated from on-chain sources as they are ingested.*

---

## Major News & Events

> *Notable events and news will be added through the wiki's source ingestion workflow as relevant articles are processed.*

---

## See Also

- [[crypto-markets]]

---

## Sources

- (Source: [[coingecko-top-1000-2026-04-09]])
