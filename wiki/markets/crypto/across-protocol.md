...
- **Root bundles + UMA optimistic oracle** — A "dataworker" periodically proposes a root bundle summarising which relayers should be reimbursed and how the HubPool should rebalance across spokes. The proposal is posted to [[uma|UMA's]] [[oracle|Optimistic Oracle]] with a bond; anyone can dispute within a challenge window. If undisputed, it executes; if disputed, UMA's dispute resolution arbitrates and a fraudulent proposer is slashed. This optimistic, dispute-based design replaces the multisig/validator trust used by most other [[cross-chain-bridge|bridges]].
