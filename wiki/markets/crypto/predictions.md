---
title: "Predictions"
type: entity
created: 2026-04-09
updated: 2026-07-16
status: draft
tags: [crypto, defi]
aliases: ["PRDT"]
entity_type: protocol
headquarters: "Decentralized"
website: "https://prdt.finance"
related: ["[[crypto-markets]]", "[[bnb]]"]
---

# Predictions

**Predictions** (PRDT) is a cryptocurrency. It ranks **#693** by market capitalization. PRDT.Finance is a decentralized prediction market platform that enables users to forecast short-term price movements of cryptocurrencies and forex pairs. Operating across multiple blockchains, the platform offers both Classic and Pro trading modes for diverse prediction experiences.

Launched in 2021, PRDT.Finance has expanded to support several EVM-compatible networks and Solana, emphasizing transparent and fast on-chain prediction rounds.

---

## Key Facts

| Field | Detail |
|---|---|
| **Ticker** | PRDT |
| **Market Cap Rank** | #693 |
| **Market Cap** | $25.96M |
| **Current Price** | $1.081 |
| **Categories** | Decentralized Finance (DeFi), BNB Chain Ecosystem, Prediction Markets |
| **Website** | [https://prdt.finance](https://prdt.finance) |
> *Market data refreshed 2026-06-12 (CoinGecko top-1000 snapshot).*

---

## Overview

PRDT.Finance is a decentralized prediction market platform that enables users to forecast short-term price movements of cryptocurrencies and forex pairs. Operating across multiple blockchains, the platform offers both Classic and Pro trading modes for diverse prediction experiences.

Launched in 2021, PRDT.Finance has expanded to support several EVM-compatible networks and Solana, emphasizing transparent and fast on-chain prediction rounds. Beyond trading, the platform features staking with daily revenue distribution to stakers, as well as partner integrations that extend its prediction markets and contribute to the ecosystem’s revenue-sharing model.

---

## Tokenomics

| Metric | Value |
|---|---|
| **Circulating Supply** | 24.00M PRDT |
| **Total Supply** | 60.00M PRDT |
| **Max Supply** | 60.00M PRDT |
| **Fully Diluted Valuation** | $65.38M |
| **Market Cap / FDV Ratio** | 0.40 |

---

## Price History

| Metric | Value |
|---|---|
| **All-Time High** | $1.26 (2025-11-07) |
| **Current vs ATH** | -13.38% |
| **All-Time Low** | $0.9976 (2025-11-01) |
| **Current vs ATL** | +9.26% |
| **24h Change** | -0.33% |
| **7d Change** | +0.23% |
| **30d Change** | -2.08% |
| **1y Change** | +0.00% |

---

## Platform & Chain Information

**Native Chain:** Binance Smart Chain

### Contract Addresses

| Chain | Address |
|---|---|
| Binance Smart Chain | `0x854b5f2bfcc5b7cd05d6259cf5d035af57cfa301` |

---

## Exchange Listings

> *No major exchange listings found in CoinGecko data.*

---

## Social & Community

| Platform | Link / Metric |
|---|---|
| **Website** | [https://prdt.finance](https://prdt.finance) |
| **Twitter** | [@PRDT_Finance](https://twitter.com/PRDT_Finance) |
| **Telegram** | [PRDT_Finance](https://t.me/PRDT_Finance) (35,022 members) |
| **GitHub** | [https://github.com/PRDTfinance](https://github.com/PRDTfinance) |
| **Whitepaper** | [https://prdt.finance/whitepaper.pdf](https://prdt.finance/whitepaper.pdf) |

---

## Trading Characteristics

| Characteristic | Detail |
|---|---|
| **24h Volume** | $75,765.00 |
| **Market Cap Rank** | #693 |
| **24h Range** | $1.09 — $1.10 |
| **Last Updated** | 2026-04-09 |

---

## Whale & Holder Information

> *On-chain holder distribution data requires blockchain analytics integration. This section will be populated from on-chain sources as they are ingested.*

---

## Major News & Events

> *Notable events and news will be added through the wiki's source ingestion workflow as relevant articles are processed.*

---

## See Also

- [[crypto-markets]]
- [[bnb]]

---

## Sources

- (Source: [[coingecko-top-1000-2026-07-16]])
