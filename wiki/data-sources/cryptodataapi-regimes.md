...
| GET | /api/v1/quant/positioning/ladder | Full >=$100k-universe liquidation ladder for one coin, by account class | symbol | Pro Plus |
...
**`/quant/positioning` and `/quant/whales`: refresh cadence, tag overlay, ladder, archive (2026-09-09):** both endpoints now carry **`positions_as_of`** (unix ms — the liq-map account-repoll cycle the book comes from) top-level and per coin. **Dedupe on `positions_as_of`, not `timestamp`**, when comparing or storing snapshots from either endpoint. `/quant/whales` moved onto the same batch-RPC liq-map cycle as `/quant/positioning` — 5 minutes once batch RPC is armed, versus the 15-minute public-path default it was on before (previously whales ran a separate, slower 15-minute cycle regardless, which is why `directional_net_usd` used to repeat unchanged across runs of three reads); `positions_as_of` makes the actual cadence self-evident per response rather than assuming a fixed interval.

`/quant/positioning` coins gain **`by_tag {smart_money, high_leverage}`** — an orthogonal overlay, NOT a breakdown of the existing `by_type` (`market_maker`/`whale`/`other`/`all`) partition: an account can carry a `by_tag` tag alongside any `by_type` class, both tags, or neither; the block zeros out when no tagged accounts are present. `symbol` also now accepts a comma-separated list (`BTC,ETH,SOL`). `/quant/whales` gains **`summary.by_tag_usd`** (same two tags) and per-coin **`top_coins[].long_pct_pctile_30d`** — the live long share ranked against that coin's own trailing 30 days (`null` until 2 days of samples exist). Both endpoints add **`meta.classifier_version`** (CalVer of the account-tagging rule set behind `by_type`/`by_tag` — a version bump can move accounts between classes with no change in the market itself; future bumps get their own changelog entry).

New **`GET /api/v1/quant/positioning/ladder?symbol=`** (Pro Plus, added to the table above) — the full >=$100k-universe liquidation ladder for one coin, binned on a 0.5%-of-mark grid: **`bins`** / **`flip`** (all account classes), **`bins_directional`** (market-maker and vault liquidations excluded — the cleaner walls, since MM liquidations are largely hedged noise), **`bins_by_class`** = `{market_maker, whale, other}` on the same grid (sums to `bins`), and **`levels`** — the 3 nearest non-empty all-class clusters above and below mark as `{price, dist_pct, total_usd, side, n_accounts}`. Same builder as the archived `liquidation_map` snapshot type (which itself gained `bins_by_class` in this release), so live and history agree.

Also in this release: two new forward-only backtesting-archive snapshot types, **`positioning`** and **`whale_activity`** (5-min cadence; no per-account history exists to backfill from — see [[cryptodataapi-backtesting]]), and `/quant/whales/history?live_only=true` now drops the ~120 modelled `source: seed` seed points, returning only observed data plus a new `meta.first_live_date` (first observed UTC day in the store).
...
**`/volatility/index/history` performance fix + retroactive correction (2026-09-17):** the endpoint used to re-read one full daily snapshot per requested day — `days=90` took ~23s and `days>=150` could outrun the edge timeout (Cloudflare 520) before finishing. It now reads a precomputed daily series and answers in well under a second across the full documented `days` range (still max 365, Pro Plus); for a minute or two right after a deploy, while that series reindexes, it can return `503 warming_up` with `Retry-After`. `cvi_realized_30`/`cvi_realized_7` history also now covers the full requested window — the volume-weighted index was only archived live from 2026-07-23, so earlier days previously had no value (or no point at all); those days are now reconstructed from daily klines with the live formula, one point per UTC day in `[today - days, today]`.

**This retroactive correction matters for anyone doing historical analysis — the corrected series now disagrees with the raw archive:** until the 2026-08-24 universe fix, the *live* index still counted delisted Binance pairs whose last candle Binance keeps serving (e.g. FTM, BNX, ALPACA carried frozen, extreme volatility and a phantom volume weight). The CVI as published 2026-07-24 → 08-23 was roughly **2x too high** (121.66 served vs 64.52 correct on 07-24; 130.04 vs 74.41 on 08-23), and `composite_score` ran ~5-9 points high over the same window. `/volatility/index/history` now serves the reconstructed, correct values for every day before 2026-08-24 — the series no longer halves overnight at the 08-24 boundary, and the reconstruction matches the live index to within ~1% on every day from 08-24 onward. Live `/volatility/index` itself was not affected after 08-24. **`/backtesting/daily-snapshots` was NOT retroactively corrected** — the raw archived snapshots are unchanged and still return exactly the originally-published (inflated, pre-08-24) numbers. Pulling pre-08-24 CVI from `/volatility/index/history` and from `/backtesting/daily-snapshots` for the same dates will silently return two different values for the same day — exactly the kind of mismatch that corrupts a backtest if the two sources get mixed. Each `/volatility/index/history` point now also carries additive **`backfilled`** (bool): `true` when any value on that day is not a verified live capture (every point before 2026-08-24, plus any later day whose archived regime block was itself backfilled; reconstructed days use today's coin universe), `false` when the values are exactly what was published that day.
