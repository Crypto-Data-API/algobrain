...
**Top-down analysis** is an investment approach that begins with the broadest economic picture and narrows progressively to individual securities: macroeconomy → sectors/industries → individual companies. It is the conceptual opposite of bottom-up analysis (which starts with a single company's fundamentals and largely ignores the macro backdrop), and it underpins most asset-allocation and [[sector-rotation]] strategies.

## The Funnel: Three Levels

Top-down analysis is often described as a funnel that filters a large universe down to a handful of holdings:
...
|-------------|-----------------|---------------------------|
| **Early expansion** | Rates low, growth accelerating | Financials, consumer discretionary, industrials, real estate |
| **Mid expansion** | Steady growth, rising rates | Technology, communications, industrials |
| **Late cycle** | Inflation up, rates peaking | Energy, materials, healthcare |
| **Recession / contraction** | Falling growth, rate cuts | Utilities, consumer staples, healthcare (defensives) |

This is the engine of [[sector-rotation]]: rotate toward cyclicals as the economy accelerates and toward defensives as it slows.

## A Worked Top-Down Walk-Through (illustrative)
...
| Dimension | Top-Down | [[bottom-up-analysis|Bottom-Up]] |
|-----------|----------|-----------|
| Starting point | Macroeconomy | Individual company |
| Primary driver | Economic cycle, rates, policy | Company fundamentals, moat, management |
| Typical practitioner | Macro funds, asset allocators, strategists | Stock-pickers, value investors |
| View of macro | Central to the thesis | Often deliberately ignored ("we can't predict the economy") |
| Risk | Wrong macro call sinks otherwise good stocks | Strong company in a collapsing sector still suffers |

Many professional processes blend the two: a top-down overlay sets allocation and sector weights, while bottom-up work selects the individual names within those weights.

## Trading Relevance

- **Asset allocation and regime positioning.** Top-down is the dominant framework for [[asset-allocation]] and tactical positioning; it tells you *what to own* before *which one*.
- **Sector rotation.** The explicit output of the second funnel level — rotating capital into sectors aligned with the current [[business-cycle]] phase.
- **Macro hedging.** A top-down view of rates, inflation, or recession risk drives index, rate, and currency hedges that a pure bottom-up picker would miss.
- **Catching systemic risk.** Because it starts with the macro, top-down analysis is better at avoiding correlated drawdowns (e.g. exiting equities broadly ahead of a recession) than bottom-up, which can be blindsided by a market-wide repricing.
- **Limitation.** Macro forecasting is notoriously unreliable; a strict top-down process can keep an investor out of strong individual stocks because the macro view was cautious. Most evidence suggests security selection (bottom-up) adds more reliable alpha than market timing (top-down), so many use top-down for risk control rather than return generation.

## Pitfalls and Risks

- **Macro forecasting is hard** — the foundational call (rates, recession, inflation) is the least reliable input; getting it wrong cascades through every downstream decision.
...
- CFA Institute curriculum, *Equity and Fixed Income — top-down vs. bottom-up approaches*.
- Investopedia, *Top-Down Investing*.
- Standard practice in macro and multi-asset portfolio management.
